...
void Chart::newDiagram(const std::string &name, const std::string &title, const std::string &unit) {
...
void Chart::output() {
  ...
  for (auto &diagram : _all_diagram) {
    ...
    ofs
      << "var diagram = new google.visualization.LineChart(document.getElementById('diagram_" << diagram->_name <<"'));\n"
    ...
    ofs
      << "var button = document.getElementById('diagram_button_" << diagram->_name << "');\n"
  ...
  for (auto &diagram : _all_diagram){
    ofs
      << "<div id=\"diagram_" << diagram->_name << "\" style=\"width: 600px; height: 400px;\"></div>\n"
      << "<input id=\"diagram_button_" << diagram->_name << "\" type=\"button\" value=\"Logarithmic scale\">\n";
  }
  ofs << "</body>\n";
  ofs << "</html>\n";
}
